        ...
        <div class="file-header">
            ...
            @if ($pengajuan->$column !== null)
                <button type="button" onclick="showModal('{{ route('kepegawaian.pengajuan.file', [
                    ...
                    'key'   => $detail->key,
                    'file'  => basename($pengajuan->{$detail->key})
        ...
        @if ($reviews->count() > 0)
            @foreach ($reviews as $version => $versionReviews)
                <div class="pengajuan-block">
                    <span class="pengajuan-label">
                        Pengajuan {{ $ordinalMap[$version] ?? 'Ke-'.$version }}
                    </span>

                  @foreach ($roleOrder as $role)
    @php
        $r = $versionReviews->first(fn($rv) => $rv->reviewer_type === $role);
    @endphp
    @if ($r)
...
<script>
    ...
    function showModal(value, title) {
        ...
        container.src = '/pdfjs/web/viewer.html?file=' + encodeURIComponent(value);
    }
